...
import io.redlink.more.observations.observationTypes.PolarHrType
...
/**
 * Heart rate derived from the Polar device's PPI recording.
 *
 * Offline mode records PPI (the device has no offline HR stream worth using here) and keeps only the
 * HR field; the live path streams HR directly.
 */
class PolarHrObservation(repos: MainRepository) : Observation(
    repos, observationType = PolarHrType(permissions)
) {
    ...
    override fun start(): Boolean {
        ...
        if (device == null) {
            Napier.d(tag = "PolarHrObservation::start") { "No Polar device connected" }
            showObservationErrorNotification(
                stringResource(R.string.observation_cannot_start),
                stringResource(R.string.observation_error)
            )
        ...
        setupJob = polarController.ensureReady(deviceId, offlineMode = offlineMode,
            ...
            onError = { error ->
                Napier.e(tag = "PolarHrObservation") { "Setup failed: ${error.message}" }
                showObservationErrorNotification(
                    stringResource(R.string.observation_cannot_start),
                    stringResource(R.string.observation_error)
                )
    ...
    private fun drainThenRestart(deviceId: String) {
        Scope.launch {
            try {
                ...
                Napier.e(tag = "PolarHrObservation") { "Failed to fetch pending offline data: ${e.message}" }
            }
    ...
    private fun storeRecording(items: List<Any>, onCompletion: () -> Unit = {}) {
        storeWindowedDirectly(
            ...
            transform = { window ->
                processHrSamples(window.filterIsInstance<PolarPpiData.PolarPpiSample>())
            },
    ...
    private fun startStreaming(deviceId: String) {
        streamingJob = Scope.launch {
            polarController.getPolarApi().startHrStreaming(deviceId)
                .catch { error ->
                    Napier.e(tag = "PolarHrObservation") { "HR stream error: ${error.message}" }
                    pauseObservation(PolarHrType(emptySet()))
                    showObservationErrorNotification(
                    ...
                    polarData.samples.firstOrNull()?.let { sample ->
                        ...
                        storeData(
                            mapOf(
                                "hr" to sample.hr,
                                "timestamp" to 0L,
                                "skinContact" to sample.contactStatus
                            )
                        )
                    }
                }
    ...
    fun processHrSamples(samples: List<PolarPpiData.PolarPpiSample>?): List<hr_data> {
        ...
        return samples.map {
            hr_data(hr = it.hr, timestamp = it.timeStamp.toLong(), skinContact = it.skinContactStatus)
        }
    }
    ...
    private companion object {
        val DATA_TYPE = PolarBleApi.PolarDeviceDataType.PPI

        /** Kept verbatim from the pre-SDK8 implementation -- the backend keys on this exact string. */
        const val DATA_KEY = "polar36q0hrdata"
    }
}
