...
@Composable
fun PolarProfileFormView(onComplete: () -> Unit) {
    val genders = PolarUserProfile.Gender.entries
    ...
    Column(
        ...
        Column(
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                genders.forEachIndexed { index, gender ->
                    ...
                    val genderColor = if (gender == PolarUserProfile.Gender.FEMALE)
        ...
        Button(
            onClick = {
                val profile = PolarUserProfile(
                    gender = genders[selectedGenderIndex],
                    age = age!!,
                    heightCm = heightCm!!,
                    weightKg = weightKg!!
                )
                PolarUserProfile.save(profile)
                onComplete()
            },
            enabled = isValid,
            colors = ButtonDefaults.buttonColors(
