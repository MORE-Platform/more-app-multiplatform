...
class Polar360Observation(repos: MainRepository):
        ...
        if (observerAccessible()){
            ...
            return polarDevices.firstOrNull()?.let{
                try {
                    ...
                    heartRateDisposable =
                        polarConnector.polarApi.startHrStreaming(it.deviceId!!).subscribeOn(Schedulers.io()).observeOn(Schedulers.io()).subscribe(
                            { polarData ->
    ...
    companion object {
        val hrReady: MutableStateFlow<Boolean> = MutableStateFlow(true)
        val tmpReady: MutableStateFlow<Boolean> = MutableStateFlow(false)
        val accReady: MutableStateFlow<Boolean> = MutableStateFlow(false)
        ...
        fun setTmpFeature(state: Boolean) {
            if (state) {
                if (!tmpReady.value) {
                    MoreApplication.shared!!.observationManager.startObservationType(
                        Polar360Type(
            ...
            tmpReady.update { state }
        }
        fun setAccFeatureß(state: Boolean) {
            if (state) {
                if (!accReady.value) {
                    MoreApplication.shared!!.observationManager.startObservationType(
                        Polar360Type(
